...
def convert_beta_folder_inductive(beta_folder, output_folder):
    """
    Convert the folder of beta dataset into the output data
    the structure of the beta folder
        indices
    ...
    kgidx = KGIndex()
    with open(osp.join(beta_folder, 'og_mappings.pkl'), 'rb') as f:
        all_mappings = pickle.load(f)
    id_to_entity, id_to_relation = all_mappings['e2id'], all_mappings['r2id']
    ...
    print("converting train KnowledgeGraph")
    train_kg = KnowledgeGraph.create(
        triple_files=osp.join(beta_folder, 'train_graph.txt'),
        kgindex=kgidx)
    print("dump converted train KnowledgeGraph")
    train_kg.dump(osp.join(output_folder, 'train_kg.tsv'))

    print("converting valid KnowledgeGraph")
    valid_whole_kg = KnowledgeGraph.create(
        triple_files=[osp.join(beta_folder, 'train_graph.txt'),
                      osp.join(beta_folder, 'val_inference.txt'),
                      osp.join(beta_folder, 'val_predict.txt')],
        kgindex=kgidx)
    valid_inference_kg = KnowledgeGraph.create(
        triple_files=[osp.join(beta_folder, 'train_graph.txt'),
                      osp.join(beta_folder, 'val_inference.txt')],
        kgindex=kgidx)
    print("dump converted valid KnowledgeGraph")
    valid_inference_kg.dump(osp.join(output_folder, 'valid_inf_kg.tsv'))
    valid_whole_kg.dump(osp.join(output_folder, 'valid_pre_kg.tsv'))

    print("converting test KnowledgeGraph")
    test_whole_kg = KnowledgeGraph.create(
        triple_files=[osp.join(beta_folder, 'train_graph.txt'),
                      osp.join(beta_folder, 'test_inference.txt'),
                      osp.join(beta_folder, 'test_inference.txt')],
        kgindex=kgidx)
    test_inference_kg = KnowledgeGraph.create(
        triple_files=[osp.join(beta_folder, 'train_graph.txt'),
                      osp.join(beta_folder, 'test_inference.txt'),
                      osp.join(beta_folder, 'test_inference.txt')],
        kgindex=kgidx)
    print("dump converted test KnowledgeGraph")
    test_inference_kg.dump(osp.join(output_folder, 'test_inf_kg.tsv'))
    test_whole_kg.dump(osp.join(output_folder, 'test_pre_kg.tsv'))


if __name__ == "__main__":
    beta_folder = "/data/zwanggc/FirstOrderQueryEstimation/data/{}"
    output_folder = "./data/{}"
    convert_beta_folder_inductive("data/FB15k-237-inductive", "data/FB15k-237-inductive")
