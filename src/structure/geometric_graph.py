...
from fol import AppFOQEstimator, ExtendedAppFOQEstimator
...
class QueryGraph(nx.MultiGraph):
    ...
    def get_node_embedding_extended(self, model: ExtendedAppFOQEstimator, node_name: str, stored_embedding_dict: dict):
        if 's' in node_name:
            ent_emb = model.get_entity_embedding(
                torch.tensor(self.node_grounded_entity_id_dict[node_name]).to(self.device))
            return ent_emb
        adj_edge_list = self.edges(node_name, data=True)
        previous_edge_dict = defaultdict(list)
        for adj_edge in adj_edge_list:
            adj_node = adj_edge[1]
            if self.ordering[adj_node] < self.ordering[node_name]:
                previous_edge_dict[adj_node].append(adj_edge)
        if len(previous_edge_dict) == 0:
            batch_size = len(self.node_grounded_entity_id_dict['s1'])  # TODO: when create the formula, it is one batch
            return model.get_whole_embedding(batch_size)
        previous_embedding_list = []
        for adj_node in previous_edge_dict:
            adj_emb = stored_embedding_dict[adj_node]
            rel_emb_list = []
            for adj_edge in previous_edge_dict[adj_node]:
                attr_dict = adj_edge[2]
                pred_name = attr_dict['name']
                rel_emb = model.get_relation_embedding(
                    torch.tensor(self.edge_grounded_entity_id_dict[pred_name]).to(self.device))
                if attr_dict['type'] == 'positive':
                    rel_emb_list.append(rel_emb)
                else:
                    rel_emb_list.append(model.relation_negation(rel_emb))
            if len(rel_emb_list) == 1:
                final_rel_emb = rel_emb_list[0]
            else:
                final_rel_emb = model.relation_conjunction(rel_emb_list)
            proj_emb = model.get_projection_embedding_extended(final_rel_emb, adj_emb)
            previous_embedding_list.append(proj_emb)
        if len(previous_embedding_list) == 1:
            final_emb = previous_embedding_list[0]
        else:
            final_emb = model.get_conjunction_embedding(previous_embedding_list)
        return final_emb
    ...
    def get_whole_graph_embedding_extended(self, model: ExtendedAppFOQEstimator):
        embedding_dict = {}
        for node in self.ordering_list:
            node_emb = self.get_node_embedding_extended(model, node, embedding_dict)
            embedding_dict[node] = node_emb
        return [embedding_dict['f1']]
