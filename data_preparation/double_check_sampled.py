...
parser.add_argument("--end_index", type=int, default=580)
...
if __name__ == "__main__":
            ...
            for sample_instance in sample_list:
                ...
                if args.use_constraint:
                    ...
                    efpo_constraint = fof.formula_list[0].deterministic_query_set(0, test_kg, neglect_negation_list,
                                                                                  True)
                    #  skip_ans = fof.formula_list[0].deterministic_query_set_with_initialization(0, test_kg, neglect_negation_list, False, efpo_constraint)
    ...
    for problem in problem_list:
        lstr, fid, qa_dict = problem
        print(lstr, fid, qa_dict)
        output_file_name = osp.join(args.output_folder,
                                    f'{args.mode}_{fid}_{args.sample_formula_scope}_qaa.json')
        os.remove(output_file_name)
        backup_file_name = osp.join('data/backup/NELL-EFOX-filtered', f'{args.mode}_{fid}_{args.sample_formula_scope}_qaa.json')
        shutil.copyfile(backup_file_name, output_file_name)
