...
def enumerate_connected_graph(e_num, f_num, connect_edge):
    ...
    nm = iso.categorical_node_match("type", "existential")
    if f_num == 1:
        for edges in combinations(list(combinations(list(range(node_num)), 2)), connect_edge):
            ...
            g_instance.add_edges_from(edges, type="positive")
            if nx.is_connected(g_instance):
                for previous_graph in all_graph:
                    if nx.is_isomorphic(previous_graph, g_instance, node_match=nm):
                        break
                else:
                    all_graph.append(g_instance)
    else:
        edge_candidate = list(combinations(list(range(node_num)), 2))
        edge_candidate.remove((e_num, e_num + 1))
        all_graph1, all_graph2 = [], []
        for edges in combinations(edge_candidate, connect_edge):
            g_instance = nx.MultiGraph()
            g_instance.add_nodes_from(list(range(e_num)), type="existential")
            g_instance.add_nodes_from(list(range(e_num, e_num + f_num)), type="free")
            g_instance.add_edges_from(edges, type="positive")
            if nx.is_connected(g_instance):
                for previous_graph in all_graph1:
                    if nx.is_isomorphic(previous_graph, g_instance, node_match=nm):
                        break
                else:
                    all_graph1.append(g_instance)
        for edges in combinations(edge_candidate, connect_edge - 1):
            g_instance = nx.MultiGraph()
            g_instance.add_nodes_from(list(range(e_num)), type="existential")
            g_instance.add_nodes_from(list(range(e_num, e_num + f_num)), type="free")
            g_instance.add_edges_from(edges, type="positive")
            if nx.is_connected(g_instance):
                for previous_graph in all_graph2:
                    if nx.is_isomorphic(previous_graph, g_instance, node_match=nm):
                        break
                else:
                    g_instance.add_edge(e_num, e_num + 1, type="positive")
                    all_graph2.append(g_instance)
        all_graph = all_graph1
        all_graph.extend(all_graph2)
    return all_graph
