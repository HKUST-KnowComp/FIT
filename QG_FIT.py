...
from Symbloic_FIT import solve_EFO1
from Neural_FIT import FIT_finetune
...
from fol import BetaEstimator4V, BoxEstimator, LogicEstimator, NLKEstimator, ConEstimator, FuzzQEstiamtor, exConEstimator
from fol import order_bounds

parser = argparse.ArgumentParser()
parser.add_argument("--config", type=str, default="config/test_EFO1/FIT_NELL_EFO1.yaml")
...
def train_step(model, opt, data_loader: QueryAnsweringMixDataLoader, loss_function):
    ...
    for formula in query_data:
        if model.name != 'FIT':
            ...
            emb_list.append(QG_embedding_batch[0])
        else:
            QG_embedding_batch = model.solve_fof(query_data[formula], 'train')
            emb_list.append(QG_embedding_batch)
    ...
    if loss_function == 'original':
        ...
        loss = -(all_positive_logit * all_subsampling_weight).sum()
        loss /= all_subsampling_weight.sum()
        positive_loss, negative_loss = None, None
    elif loss_function == 'weight_BCE':
        all_subsampling_weight = all_subsampling_weight.unsqueeze(-1)
        criterion = torch.nn.BCELoss(weight=all_subsampling_weight, reduction='sum')
        loss = criterion(all_positive_logit, all_negative_logit) / (torch.sum(all_subsampling_weight) * model.n_entity)
        positive_loss, negative_loss = None, None
    elif loss_function == 'BCE':
        criterion = torch.nn.BCELoss()
        loss = criterion(all_positive_logit, all_negative_logit)
        positive_loss, negative_loss = None, None
    else:
    ...
    with torch.no_grad():
        ...
        for ifof, fof in t:
            if model.name != 'FIT':
                ...
                QG_embedding_list = [model.solve_fof(fof, 'test')]
        ...
        if not all_one_log and not all_no_log:
            all_metrics = {formula: all_two_log}
        else:
            all_metrics = {formula: [all_two_log, all_one_log, all_no_log]}
        writer.save_pickle(all_metrics, f"all_logging_test_{step}_{formula_id}.pickle")
        return all_metrics[formula]
...
if __name__ == "__main__":
    ...
    if model_name == 'beta':
        ...
        allowed_norm = ['DeMorgan+MultiI', 'DNF+MultiIU']
    elif model_name == 'exConE':
        model = exConEstimator(**model_params)
    elif model_name == 'FuzzQE':
        model = FuzzQEstiamtor(**model_params)
    elif model_name == 'FIT':
        model_params['train_kg'] = train_kg
        model = FIT_finetune(**model_params)
    else:
    ...
    if 'train' in configure['action']:
        ...
        train_formulas = train_formula_file['formula'].tolist()
        # train_all_tm = QueryAnsweringMixDataLoader(
        #     train_data_file,
        #     target_lstr=train_formulas,
        ...
        if train_config['formula_split'] == 'path':
            ...
            if train_path_formula_list:
                train_primary_tm = QueryAnsweringMixDataLoader(
                    train_data_file,
                    target_lstr=train_path_formula_list,
                    size_limit=train_config['size_limit'],
                    batch_size=train_config['batch_size'],
                    shuffle=False,
                    num_workers=configure['data']['cpu'])
            if train_other_formula_list:
                train_other_tm = QueryAnsweringMixDataLoader(
                    ...
                    shuffle=False,
                    num_workers=configure['data']['cpu'])
        elif train_config['formula_split'] == 'all':
            train_primary_tm = QueryAnsweringMixDataLoader(
                train_data_file,
                target_lstr=train_formulas,
                batch_size=train_config['batch_size'],
                shuffle=False,
                num_workers=configure['data']['cpu'])
        elif train_config['formula_split'] == 'sign':
            train_positive_list, train_negative_list = [], []
            for train_formula in train_formulas:
                if '!' in train_formula:
                    train_negative_list.append(train_formula)
                else:
                    train_positive_list.append(train_formula)
            if train_positive_list:
                train_primary_tm = QueryAnsweringMixDataLoader(
                    train_data_file,
                    target_lstr=train_positive_list,
                    batch_size=train_config['batch_size'],
                    shuffle=False,
                    num_workers=configure['data']['cpu'])
            if train_negative_list:
                train_other_tm = QueryAnsweringMixDataLoader(
                    train_data_file,
                    target_lstr=train_negative_list,
                    batch_size=train_config['batch_size'],
                    shuffle=False,
                    num_workers=configure['data']['cpu'])
        else:
    ...
    prefix = configure['data']['data_prefix']
    training_logs = []
        ...
        for step in t:
            ...
            if 'train' in configure['action']:
                if step >= train_config['warm_up_steps']:
                    ...
                    # logging
                if model_name == 'FIT':
                    _log = train_step(model, opt, train_primary_tm, loss_function)
                    if train_other_tm:
                    ...
                    if train_other_tm:
                        ...
                        if model_name != 'FuzzQE':
                            ...
                            _log_other_2 = train_step(model, opt, train_other_tm, loss_function)
                _alllog = {}
                for key in _log:
                    _alllog[f'all_{key}'] = (_log[key] + _log_other[key]) / 2 if _log_other else _log[key]
            ...
            if step % train_config['evaluate_every_steps'] == 0 or step == train_config['steps']:
                if 'valid' in configure['action']:
                    ...
                    for i, row in tqdm.tqdm(all_formula_data.iterrows(), total=len(all_formula_data)):
                        ...
                        formula = row['formula']
                        data_path = osp.join(configure['data']['data_folder'], f'valid_{formula_id}_{prefix}_qaa.json')
                        if not osp.exists(data_path):
                            print(f'Warnings,{data_path} not exists!')
                            continue
                        two_mar_log = eval_step(data_path, model, configure, device, step, writer)
                        for metric in two_mar_log:
                            if metric != 'num_queries':
                                two_mar_log[metric] /= two_mar_log['num_queries']
                        all_metrics[formula] = two_mar_log
                    writer.save_pickle(all_metrics, f"all_logging_valid_{step}.pickle")
